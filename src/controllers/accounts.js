...
class AccountsController {
  ...
  static async getAccountsInfo(req, res, next) {
    try {
      const result = await AccountsService.getAccountsInfo({id: req.query.id});
      return responseHandler({
        response: res,
        result,
      });
  ...
  static async updateBalance(req, res, next) {
    try {
      const result = await AccountsService.updateBalance({id: req.query.id, balance: req.body.balance});
      return responseHandler({
        response: res,
        result,
      });
